...
let mockAcceptedDraftWords = 0;
let mockAcceptedFinalWords = 0;
let mockAcceptedCount = 0;
let mockRejectedCount = 0;
...
for (let index = 0; index < sample; index += 1) {
  ...
  if (writerMode === 'mock_llm') {
    ...
    const rejected = writer?.warnings?.includes('mock_rewrite_rejected') || writer?.critique.observations?.includes('mock_rewrite_rejected');
    ...
    if (rejected) {
      ...
      mockAcceptedCount += 1;
      mockAcceptedDraftWords += draftCount;
      mockAcceptedFinalWords += result.compiledPrompt.wordCount;
    }
...
const mockAcceptedCompression = mockAcceptedCount ? Number(((1 - mockAcceptedFinalWords / mockAcceptedDraftWords) * 100).toFixed(2)) : 0;
const metrics = {
  sample,
  ...
  acceptedDraftFinalCompressionPercent: mockAcceptedCompression,
...
if (metrics.mockAcceptedCount > 0 && (metrics.acceptedDraftFinalCompressionPercent < 15 || metrics.acceptedDraftFinalCompressionPercent > 25)) failures.push(`accepted mock compression ${metrics.acceptedDraftFinalCompressionPercent}%`);
