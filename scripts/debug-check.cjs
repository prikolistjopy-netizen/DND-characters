...
const { generateCharacterSeed, validateGeneratedSeed, resetSmartCandidatePoolMemory, resolveArtDirection, validateArtDirectionBrief, renderTextureHygieneRisk, visualDirectorRisk } = require(path.join(outDir, 'lib/generator.js'));
...
function promptHasPrimaryRead(prompt, seed) {
  ...
  const weaponName = String(seed.weapon.name).toLowerCase().split(/\s+/)[0];
  return {
    race: lower.includes(seed.race.name.toLowerCase()) || (seed.race.name === 'dwarf' && /dwarf|beard|compact/.test(lower)),
    classRead: lower.includes(seed.primaryClass.toLowerCase()),
    silhouette: /silhouette|body|shape|masses|build/.test(lower),
    weapon: lower.includes(weaponName) || /primary tool|weapon|blade|staff|bow|hammer|focus|instrument|shield|knife|rapier/.test(lower),
    pose: /stance|pose|composition|turn|carries|holds|walk/.test(lower),
...
if (imagePromptMissingRaceAppearanceCount !== 0) failures.push(`image prompts missing race identity in artist prompt: ${imagePromptMissingRaceAppearanceCount}`);
if (imagePromptMissingClassReadabilityCount !== 0) failures.push(`image prompts missing class identity in artist prompt: ${imagePromptMissingClassReadabilityCount}`);
...
// Artist prompts intentionally use a single restrained aasimar marker and suppress halo stacks; legacy marker-ratio failures are no longer applicable.
...
printStats('Gender presentation distribution', genderPresentationDistribution);
console.log(`All-masculine 12-sample batch risk count: ${allMasculineBatchRiskCount}`);
console.log(`Beard overuse count: ${beardOveruseCount}`);
console.log(`Elder overuse count: ${elderOveruseCount}`);
printStats('Face archetype distribution', faceArchetypeDistribution);
printStats('Body type distribution', bodyTypeDistribution);
printStats('Backdrop lane distribution', backdropLaneDistribution);
console.log(`Empty background risk count: ${emptyBackgroundRiskCount}`);
console.log(`Repeated backdrop lane within 8: ${repeatedBackdropLaneWithin8}`);
console.log(`Scene prop regression count: ${scenePropRegressionCount}`);
printStats('Composition lane distribution', compositionLaneDistribution);
console.log(`Frontal iconic overuse count: ${frontalIconicOveruseCount}`);
console.log(`Repeated composition within 8: ${repeatedCompositionWithin8}`);
console.log(`Silhouette variety score: ${(silhouetteVarietyScoreTotal / sampleSize).toFixed(2)}`);
console.log(`Pose composition mismatch count: ${poseCompositionMismatchCount}`);
console.log(`Visible prop budget exceeded count: ${visiblePropBudgetExceededCount}`);
console.log(`Belt clutter risk count: ${beltClutterRiskCount}`);
console.log(`Paper/map/compass leak count: ${paperMapCompassLeakCount}`);
console.log(`Rogue map/compass primary count: ${rogueMapCompassPrimaryCount}`);
console.log(`Fighter focus object count: ${fighterFocusObjectCount}`);
console.log(`Artificer prop soup risk count: ${artificerPropSoupRiskCount}`);
console.log(`Story detail object leak count: ${storyDetailObjectLeakCount}`);
console.log(`Fighter-paladin hard combo repaired/unrepaired: ${fighterPaladinHardComboRepairedCount}/${Math.max(0, fighterPaladinHardComboCount - fighterPaladinHardComboRepairedCount)}`);
console.log(`Rogue-bard hard combo repaired/unrepaired: ${rogueBardHardComboRepairedCount}/${Math.max(0, rogueBardHardComboCount - rogueBardHardComboRepairedCount)}`);
console.log(`Wizard-cleric hard combo count: ${wizardClericHardComboCount}`);
console.log(`Druid-bard hard combo count: ${druidBardHardComboCount}`);
console.log(`Noisy texture prompt risk count: ${noisyTexturePromptRiskCount}`);
console.log(`Heavy grain risk count: ${heavyGrainRiskCount}`);
console.log(`Microdetail overuse prompt risk count: ${microdetailOverusePromptRiskCount}`);
console.log(`Render hygiene phrase coverage: ${renderHygienePhraseCoverage}/${sampleSize}`);
console.log(`Repeated visual lane combo within 8: ${repeatedVisualLaneComboWithin8}`);
console.log(`Repeated class pose backdrop within 12: ${repeatedClassPoseBackdropWithin12}`);
console.log(`Visual diversity score: ${((genderPresentationDistribution.size + faceArchetypeDistribution.size + bodyTypeDistribution.size + backdropLaneDistribution.size + compositionLaneDistribution.size) / 5).toFixed(2)}`);
