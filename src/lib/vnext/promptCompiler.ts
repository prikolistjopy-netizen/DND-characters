...
function lintPrompt(prompt: string, negativePrompt: string) {
  ...
  if (/shows tension|visual tension|meaningful tension|class stereotype|pressure presses at the edge of attention/i.test(prompt)) warnings.push('abstract or meta tension without carrier');
...
export function compilePrompt(seed: SemanticSeed, visual: VisualDirection, maxWords = 320): CompiledPrompt {
  const parts = [
    `Cinematic painted fantasy character concept, full-body ${label(seed.identity.speciesId)} ${label(seed.identity.classId)} ${seed.identity.profession}.`,
    `${visual.embodiment.proportions}; ${visual.embodiment.silhouette}.`,
    `${visual.embodiment.posture}. ${visual.embodiment.gesture}.`,
    `${visual.scene.currentMoment}; failure would mean ${visual.scene.stakes}.`,
    `The visible obstacle is ${visual.scene.activeObstacle}; attention stays on ${visual.scene.subjectOfAction}.`,
    `${visual.embodiment.expression}; ${visual.embodiment.gaze}.`,
    `${visual.life.clothing}, ${visual.life.materials}.`,
    `Primary tool: ${visual.life.primaryTool}; ${visual.life.handling}.`,
    `${visual.life.livedInTrace}; repairs and stains stay localized and functional.`,
    `${seed.currentMoment.visualConsequence}.`,
    `${visual.power.manifestation}; power stays below face, tool, and action.`,
    `${visual.scene.environment}; ${visual.artDirection.lighting}.`,
    `${visual.artDirection.paletteRoles.join('; ')}.`,
    `${visual.artDirection.composition}; ${visual.artDirection.camera}; focal order ${visual.artDirection.focalOrder.join(' to ')}.`,
