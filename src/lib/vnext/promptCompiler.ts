...
export class MockLlmPromptWriter extends LocalHeuristicPromptWriter {
  ...
  rewrite(input: PromptWriterInput, draftPrompt: string, critique: PromptCritique): PromptWriterOutput {
    const localFallback = new LocalHeuristicPromptWriter().rewrite(input, draftPrompt, critique);
    const sentences = splitSentences(stripInternalLanguage(draftPrompt));
    ...
    const postCritique = critiquePrompt(finalPrompt, promptPlan, input.semanticSeed);
    const draftProfessionMentions = (draftPrompt.toLowerCase().match(new RegExp(`\\b${input.semanticSeed.identity.profession.toLowerCase()}\\b`, 'g')) || []).length;
    const finalProfessionMentions = (finalPrompt.toLowerCase().match(new RegExp(`\\b${input.semanticSeed.identity.profession.toLowerCase()}\\b`, 'g')) || []).length;
    const compression = (countWords(draftPrompt) - countWords(finalPrompt)) / Math.max(1, countWords(draftPrompt));
    const rejectionReasons: string[] = [];
    if (!postCritique.dominantAnchorClear) rejectionReasons.push('dominant anchor visual carrier removed');
    if (!postCritique.classReadable) rejectionReasons.push('class evidence removed');
    if (!postCritique.speciesReadable) rejectionReasons.push('species morphology removed');
    if (!postCritique.subjectRoleClear) rejectionReasons.push('subject role removed');
    if (!postCritique.obstacleRoleClear) rejectionReasons.push('obstacle role removed');
    if (!postCritique.conflictCarrierVisible) rejectionReasons.push('conflict carrier removed');
    if (!postCritique.sceneClear) rejectionReasons.push('scene clarity removed');
    if (finalProfessionMentions > draftProfessionMentions) rejectionReasons.push('profession weight increased');
    if (input.semanticSeed.life.professionSalience !== 'dominant' && postCritique.professionOverweight) rejectionReasons.push('profession budget violation');
    if (compression < 0.12 || compression > 0.25) rejectionReasons.push(`compression outside accepted range:${Math.round(compression * 100)}%`);
    if (rejectionReasons.length) {
      return {
        ...localFallback,
        critique: { ...critique, observations: [...critique.observations, 'mock_rewrite_rejected', ...rejectionReasons] },
        removedDetails,
        warnings: [...new Set([...localFallback.warnings, 'mock_rewrite_rejected', ...rejectionReasons])],
      };
    }
    ...
    return { promptPlan, draftPrompt, critique: { ...postCritique, observations }, finalPrompt, negativePrompt, removedDetails, priorityCompliance: input.semanticDirectorPlan.priorityCompliance, warnings: lintPrompt(finalPrompt, negativePrompt) };
  }
}
...
export function compilePrompt(seed: SemanticSeed, visual: VisualDirection, maxWords = HARD_MAX_WORDS, semanticPlan = directSemantic(seed), writerMode: PromptWriterMode = 'local'): CompiledPrompt {
  ...
  const promptWriter: PromptWriterResult = {
    promptPlan,
    draftPrompt,
    critique: { ...writerOutput.critique, compressionRatio: draftPrompt ? 1 - (countWords(prompt) / countWords(draftPrompt)) : 0 },
    finalPrompt: prompt,
    writerMode,
    negativePrompt,
    warnings: [...new Set([...warnings, ...writerOutput.warnings])],
