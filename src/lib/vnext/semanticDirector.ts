...
export function directSemantic(seed: SemanticSeed): SemanticDirectorPlan {
  ...
  return {
    ...
    anchorInterpretation: seed.priorityPlan.anchorInterpretation,
    sceneStrategy: seed.priorityPlan.sceneStrategy,
    conflictCarrier: seed.priorityPlan.conflictCarrier,
    ...
    sceneFocus: `${seed.priorityPlan.sceneStrategy} around ${seed.currentMoment.goal} against ${seed.currentMoment.obstacle}`,
    emotionalFocus: seed.psychology.contradiction,
    emotionalAxis: `${seed.priorityPlan.anchorInterpretation} / ${seed.psychology.emotionalRestraint}`,
    visualConflict: `${seed.priorityPlan.conflictCarrier} carries ${dominantNarrativeAnchor}`,
    actionTiming: seed.priorityPlan.sceneStrategy,
    subjectRole: seed.currentMoment.dependent,
    obstacleRole: seed.currentMoment.obstacle,
    obstacleExpression: seed.currentMoment.obstacleExpression,
    environmentRole: seed.world.environmentRole,
    powerBudget: seed.power.visibility === 'full_apparition' ? 'rare background manifestation only' : 'one local cue subordinate to action',
    powerConflictMode: seed.power.conflictMode,
    powerDecision: seed.power.decision,
    powerEvidenceChannel: seed.power.evidenceChannel,
    ...
    priorityCompliance: [
      ...
      `interpretation:${seed.priorityPlan.anchorInterpretation}`,
      `scene strategy:${seed.priorityPlan.sceneStrategy}`,
      `conflict carrier:${seed.priorityPlan.conflictCarrier}`,
