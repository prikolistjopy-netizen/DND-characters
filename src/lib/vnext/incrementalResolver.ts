import type { AgeBand, ConflictCarrier, EnvironmentRole, GenderPresentation, ObstacleExpression, PilotClassId, PilotSpeciesId, PowerConflictMode, PowerDecision, PowerEvidenceChannel, PowerVisibility, ProfessionSalience, SceneStrategy, ScoreTraceEntry, SemanticAnchor, SemanticSeed, VNextInput } from './contracts';
...
const obstacleExpressions: ObstacleExpression[] = ['physical_barrier', 'social_refusal', 'institutional_rule', 'withheld_information', 'damaged_object', 'unstable_environment', 'time_limit', 'dependent_resistance', 'public_scrutiny', 'internal_block'];
const environmentRoles: EnvironmentRole[] = ['neutral_stage', 'practical_workspace', 'social_pressure', 'physical_hazard', 'emotional_contrast', 'witness_space', 'transitional_space', 'private_refuge', 'public_exposure', 'aftermath_space'];
const powerConflictModes: PowerConflictMode[] = ['possession', 'obligation', 'contamination', 'dependency', 'borrowed_authority', 'unstable_control', 'forbidden_knowledge', 'harmful_side_effect', 'social_stigma', 'moral_prohibition', 'mistaken_identity', 'institutional_ban'];
const powerDecisions: PowerDecision[] = ['use', 'refuse', 'conceal', 'transfer', 'contain', 'reveal', 'bargain', 'endure', 'redirect', 'destroy', 'surrender', 'exploit'];
const powerEvidenceChannels: PowerEvidenceChannel[] = ['behavioral', 'bodily', 'object', 'relational', 'environmental', 'reflected', 'social', 'absence', 'consequence', 'witness_reaction'];
...
export function resolveSemanticSeed(input: VNextInput): SemanticSeed {
  ...
  const obstacleExpression = pick(obstacleExpressions, rng, 'moment.obstacleExpression', scoreTrace, (item) => {
    if (item === 'physical_barrier' && /locked threshold|failing structure/i.test(obstacle)) return 12;
    if (item === 'public_scrutiny' && /crowd|witness|suspicion|expectation/i.test(obstacle)) return 16;
    if (item === 'unstable_environment' && /weather/i.test(obstacle)) return 18;
    if (item === 'institutional_rule' && priority.conflictCarrier === 'institution') return 14;
    return 0;
  });
  const environmentRole = pick(environmentRoles, rng, 'world.environmentRole', scoreTrace, (item) => {
    if (item === 'witness_space' && priority.conflictCarrier === 'public_judgment') return 16;
    if (item === 'physical_hazard' && obstacleExpression === 'unstable_environment') return 18;
    if (item === 'transitional_space' && priority.sceneStrategy === 'movement_through_space') return 12;
    if (item === 'social_pressure' && priority.sceneStrategy === 'social_exchange') return 12;
    return 0;
  });
  const powerConflictMode = pick(powerConflictModes, rng, 'power.conflictMode', scoreTrace, (item) => {
    const classBoosts: Record<string, Partial<Record<PowerConflictMode, number>>> = {
      warlock: { obligation: 18, dependency: 14, borrowed_authority: 14, unstable_control: 10 },
      cleric: { moral_prohibition: 16, obligation: 12, institutional_ban: 10 },
      paladin: { institutional_ban: 16, moral_prohibition: 14, obligation: 10 },
      wizard: { forbidden_knowledge: 18, unstable_control: 12 },
      druid: { contamination: 16, harmful_side_effect: 12 },
      rogue: { borrowed_authority: 12, mistaken_identity: 12, social_stigma: 10 },
      barbarian: { harmful_side_effect: 14, possession: 10, unstable_control: 10 },
      monk: { dependency: 12, contamination: 10, moral_prohibition: 10 },
    };
    return classBoosts[classId]?.[item] ?? 0;
  });
  const powerDecision = pick(powerDecisions, rng, 'power.decision', scoreTrace, (item) => {
    const classBoosts: Record<string, Partial<Record<PowerDecision, number>>> = {
      warlock: { bargain: 18, refuse: 12, conceal: 10 },
      cleric: { use: 8, reveal: 10, endure: 8 },
      paladin: { refuse: 12, contain: 10, surrender: 8 },
      wizard: { contain: 12, destroy: 10, exploit: 6 },
      druid: { redirect: 12, contain: 10, refuse: 8 },
      rogue: { conceal: 12, exploit: 10, transfer: 8 },
      barbarian: { endure: 12, redirect: 10, refuse: 8 },
      monk: { contain: 12, surrender: 10, endure: 8 },
    };
    return classBoosts[classId]?.[item] ?? 0;
  });
  const powerEvidenceChannel = pick(powerEvidenceChannels, rng, 'power.evidenceChannel', scoreTrace, (item) => {
    if (item === 'object' && visibility === 'object') return 15;
    if (item === 'reflected' && visibility === 'reflected') return 15;
    if (item === 'bodily' && visibility === 'bodily') return 15;
    if (item === 'absence' && visibility === 'latent') return 10;
    return 0;
  });
